...
async function loadFonts() {
  ...
  // Which families Google Fonts can serve, so a visitor can preview a font they
  // have not installed. Generated by build/make-webfonts.js.
  let web = { families: [], aliases: {} };
  try {
    const response = await fetch('assets/webfonts.json');
    if (response.ok) web = await response.json();
  } catch {
    // The list still works from locally installed fonts alone.
  }
  ...
  const rows = catalogue.map((font) => {
    ...
    return {
      ...
      installed,
      source,
      // What actually paints this row: the font itself when we have it, the
      // unpatched base family for a Nerd Font, otherwise nothing.
  ...
  // Only fetch a web font once its row is actually on screen.
  ...
  for (const row of rows) {
    ...
    if (usableRow) {
      ...
      // Clicking a font down here changes the demo further up the page, so bring
      // the demo back into view or the click looks like it did nothing.
...
function setUpDemo(usableFonts) {
  ...
  // Split the picker so it is obvious which fonts are yours and which ones the
  // page is fetching so you can try them anyway.
  const labels = {
  ...
  applyDemoFont = (name, { reveal = false } = {}) => {
    ...
    // A web font is only requested when its row scrolls past, so a font picked
    // from the dropdown may not have been fetched yet.
